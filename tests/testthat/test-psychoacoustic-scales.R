test_that("hz_to_bark works with numeric input", {
  skip_if_not_installed("superassp")

  # Test single value
  bark <- hz_to_bark(1000, as_units = FALSE)
  ...
  bark_vals <- hz_to_bark(freqs, as_units = FALSE)
...
test_that("hz_to_bark validates input", {
  skip_if_not_installed("superassp")

  # Negative frequencies should error
  expect_error(hz_to_bark(-100, as_units = FALSE))

  # NA values should be preserved
  result <- hz_to_bark(c(1000, NA, 2000), as_units = FALSE)
  expect_true(is.na(result[2]))
  expect_false(is.na(result[1]))
})


test_that("hz_to_bark methods produce reasonable results", {
  skip_if_not_installed("superassp")

  freq <- 1000
  bark_traun <- hz_to_bark(freq, method = "traunmuller", as_units = FALSE)
  bark_zwicker <- hz_to_bark(freq, method = "zwicker", as_units = FALSE)
  bark_wang <- hz_to_bark(freq, method = "wang", as_units = FALSE)
...
test_that("hz_to_bark works with units package", {
  skip_if_not_installed("superassp")
...
test_that("bark_to_hz works with numeric input", {
  skip_if_not_installed("superassp")

  # Test single value
  hz <- bark_to_hz(10, as_units = FALSE)
  ...
  freqs <- bark_to_hz(bark_vals, as_units = FALSE)
...
test_that("bark_to_hz validates input", {
  skip_if_not_installed("superassp")

  # Out of range values should warn
  expect_warning(bark_to_hz(30, as_units = FALSE))
  expect_warning(bark_to_hz(-5, as_units = FALSE))

  # NA values should be preserved
  result <- bark_to_hz(c(10, NA, 15), as_units = FALSE)
  expect_true(is.na(result[2]))
  expect_false(is.na(result[1]))
})


test_that("bark_to_hz methods produce reasonable results", {
  skip_if_not_installed("superassp")

  bark <- 10
  hz_traun <- bark_to_hz(bark, method = "traunmuller", as_units = FALSE)
  hz_zwicker <- bark_to_hz(bark, method = "zwicker", as_units = FALSE)
  hz_wang <- bark_to_hz(bark, method = "wang", as_units = FALSE)
...
test_that("bark_to_hz works with units package", {
  skip_if_not_installed("superassp")
  skip_if_not_installed("units")
...
test_that("round-trip conversion preserves values (Traunmüller)", {
  ...
  for (freq in test_freqs) {
    bark <- hz_to_bark(freq, method = "traunmuller", as_units = FALSE)
    freq_recovered <- bark_to_hz(bark, method = "traunmuller", as_units = FALSE)
...
test_that("round-trip conversion preserves values (Wang)", {
  ...
  for (freq in test_freqs) {
    bark <- hz_to_bark(freq, method = "wang", as_units = FALSE)
    freq_recovered <- bark_to_hz(bark, method = "wang", as_units = FALSE)
...
test_that("round-trip conversion preserves values (Zwicker)", {
  ...
  for (freq in test_freqs) {
    bark <- hz_to_bark(freq, method = "zwicker", as_units = FALSE)
    freq_recovered <- bark_to_hz(bark, method = "zwicker", as_units = FALSE)
...
test_that("units integration works end-to-end", {
  ...
  # Convert back to Hz
  freq_recovered <- bark_to_hz(bark)
...
test_that("conversion handles edge cases", {
  ...
  # Very low frequency
  bark_low <- hz_to_bark(20, as_units = FALSE)
  expect_true(bark_low >= 0)

  # Very high frequency
  bark_high <- hz_to_bark(15000, as_units = FALSE)
  expect_true(bark_high <= 24)

  # Zero frequency (boundary case)
  bark_zero <- hz_to_bark(0, as_units = FALSE)
...
test_that("methods parameter is validated", {
  ...
  expect_error(hz_to_bark(1000, method = "invalid"))
  expect_error(bark_to_hz(10, method = "invalid"))
})
...
test_that("all methods produce monotonic mappings", {
  ...
  for (method in methods) {
    bark_vals <- hz_to_bark(test_freqs, method = method, as_units = FALSE)
...
test_that("as_units parameter controls output format", {
  ...
  bark_numeric <- hz_to_bark(freq, as_units = FALSE)
  ...
  bark_units <- hz_to_bark(1000, as_units = TRUE)
  expect_s3_class(bark_units, "units")

  # Same for inverse
  freq_numeric <- bark_to_hz(10, as_units = FALSE)
  expect_type(freq_numeric, "double")

  freq_units <- bark_to_hz(10, as_units = TRUE)
...
test_that("hz_to_erb works with numeric input", {
  skip_if_not_installed("superassp")

  erb <- hz_to_erb(1000, as_units = FALSE)
  ...
  # Vector input
  freqs <- c(100, 500, 1000, 2000, 4000)
  erb_vals <- hz_to_erb(freqs, as_units = FALSE)
  expect_length(erb_vals, 5)
  expect_true(all(diff(erb_vals) > 0))  # Monotonic
})


test_that("hz_to_erb methods produce reasonable results", {
  skip_if_not_installed("superassp")

  freq <- 1000
  erb_glasberg <- hz_to_erb(freq, method = "glasberg1990", as_units = FALSE)
  erb_moore <- hz_to_erb(freq, method = "moore1983", as_units = FALSE)
...
test_that("hz_to_erb works with units package", {
  skip_if_not_installed("superassp")
  skip_if_not_installed("units")
...
test_that("erb_to_hz works with numeric input", {
  skip_if_not_installed("superassp")

  hz <- erb_to_hz(15, as_units = FALSE)
  expect_type(hz, "double")
  expect_true(hz > 0)
})
...
test_that("round-trip ERB conversion preserves values (Glasberg)", {
  ...
  for (freq in test_freqs) {
    erb <- hz_to_erb(freq, method = "glasberg1990", as_units = FALSE)
    freq_recovered <- erb_to_hz(erb, method = "glasberg1990", as_units = FALSE)
...
test_that("hz_to_mel works with numeric input", {
  skip_if_not_installed("superassp")

  mel <- hz_to_mel(1000, as_units = FALSE)
  ...
  # Vector input
  freqs <- c(100, 500, 1000, 2000, 4000)
  mel_vals <- hz_to_mel(freqs, as_units = FALSE)
  expect_length(mel_vals, 5)
  expect_true(all(diff(mel_vals) > 0))
})


test_that("hz_to_mel methods produce reasonable results", {
  skip_if_not_installed("superassp")

  freq <- 1000
  mel_htk <- hz_to_mel(freq, method = "htk", as_units = FALSE)
  mel_slaney <- hz_to_mel(freq, method = "slaney", as_units = FALSE)
...
test_that("hz_to_mel works with units package", {
  skip_if_not_installed("superassp")
  skip_if_not_installed("units")
...
test_that("mel_to_hz works with numeric input", {
  skip_if_not_installed("superassp")

  hz <- mel_to_hz(1000, as_units = FALSE)
  expect_type(hz, "double")
  expect_true(hz > 0)
})
...
test_that("round-trip mel conversion preserves values (HTK)", {
  ...
  for (freq in test_freqs) {
    mel <- hz_to_mel(freq, method = "htk", as_units = FALSE)
    freq_recovered <- mel_to_hz(mel, method = "htk", as_units = FALSE)
...
test_that("round-trip mel conversion preserves values (Slaney)", {
  ...
  for (freq in test_freqs) {
    mel <- hz_to_mel(freq, method = "slaney", as_units = FALSE)
    freq_recovered <- mel_to_hz(mel, method = "slaney", as_units = FALSE)
...
test_that("hz_to_semitone works with numeric input", {
  skip_if_not_installed("superassp")

  # 880 Hz is 12 semitones above 440 Hz (A4 to A5 = 1 octave)
  st <- hz_to_semitone(880, ref_freq = 440, as_units = FALSE)
  expect_type(st, "double")
  expect_equal(st, 12, tolerance = 0.001)

  # 440 Hz relative to itself should be 0
  st_zero <- hz_to_semitone(440, ref_freq = 440, as_units = FALSE)
  expect_equal(st_zero, 0, tolerance = 0.001)

  # Vector input
  freqs <- c(440, 466.16, 493.88, 523.25, 554.37, 587.33)  # A4 to D5
  st_vals <- hz_to_semitone(freqs, ref_freq = 440, as_units = FALSE)
  expect_length(st_vals, 6)
})


test_that("hz_to_semitone handles different reference frequencies", {
  skip_if_not_installed("superassp")

  # Middle C (261.63 Hz) to C one octave higher (523.25 Hz)
  st <- hz_to_semitone(523.25, ref_freq = 261.63, as_units = FALSE)
  ...
  st_units <- hz_to_semitone(freq, ref_freq = ref, as_units = TRUE)
  expect_s3_class(st_units, "units")
  expect_equal(as.numeric(st_units), 12, tolerance = 0.001)
})


test_that("hz_to_semitone works with units package", {
  skip_if_not_installed("superassp")
  skip_if_not_installed("units")
  ...
  freq <- set_units(880, Hz)
  st <- hz_to_semitone(freq, ref_freq = 440)

  expect_s3_class(st, "units")
})


test_that("semitone_to_hz works with numeric input", {
  skip_if_not_installed("superassp")

  # 12 semitones above 440 Hz should be 880 Hz
  hz <- semitone_to_hz(12, ref_freq = 440, as_units = FALSE)
  expect_type(hz, "double")
  expect_equal(hz, 880, tolerance = 0.01)

  # 0 semitones should return reference frequency
  hz_zero <- semitone_to_hz(0, ref_freq = 440, as_units = FALSE)
  expect_equal(hz_zero, 440)

  # Negative semitones (going down)
  hz_down <- semitone_to_hz(-12, ref_freq = 440, as_units = FALSE)
...
test_that("round-trip semitone conversion preserves values", {
  ...
  for (freq in test_freqs) {
    st <- hz_to_semitone(freq, ref_freq = 440, as_units = FALSE)
    freq_recovered <- semitone_to_hz(st, ref_freq = 440, as_units = FALSE)
...
test_that("semitone conversion validates input", {
  ...
  expect_error(hz_to_semitone(0, ref_freq = 440))
  expect_error(hz_to_semitone(-100, ref_freq = 440))
  expect_error(hz_to_semitone(100, ref_freq = -440))
})
...
test_that("musical intervals are correct", {
  ...
  freq_fifth <- semitone_to_hz(7, ref_freq = 440, as_units = FALSE)
  expect_equal(freq_fifth / 440, 2^(7/12), tolerance = 0.001)

  # Perfect fourth (5 semitones)
  freq_fourth <- semitone_to_hz(5, ref_freq = 440, as_units = FALSE)
  expect_equal(freq_fourth / 440, 2^(5/12), tolerance = 0.001)

  # Major third (4 semitones)
  freq_third <- semitone_to_hz(4, ref_freq = 440, as_units = FALSE)
...
test_that("hz_to_semitone works with UEP83 reference", {
  skip_if_not_installed("superassp")

  # UEP83 uses 110 Hz (A2) as reference
  # 220 Hz is one octave (12 semitones) above 110 Hz
  st <- hz_to_semitone(220, ref_source = "UEP83", as_units = FALSE)
  expect_equal(st, 12, tolerance = 0.001)

  # 110 Hz relative to itself should be 0
  st_zero <- hz_to_semitone(110, ref_source = "UEP83", as_units = FALSE)
  expect_equal(st_zero, 0, tolerance = 0.001)
})


test_that("hz_to_semitone works with Praat reference", {
  skip_if_not_installed("superassp")

  # Praat uses 100 Hz as arbitrary reference
  # 200 Hz is one octave (12 semitones) above 100 Hz
  st <- hz_to_semitone(200, ref_source = "Praat", as_units = FALSE)
  expect_equal(st, 12, tolerance = 0.001)

  # 100 Hz relative to itself should be 0
  st_zero <- hz_to_semitone(100, ref_source = "Praat", as_units = FALSE)
  expect_equal(st_zero, 0, tolerance = 0.001)
})


test_that("hz_to_semitone works with A4 reference (default)", {
  skip_if_not_installed("superassp")

  # A4 uses 440 Hz as reference (default)
  # 880 Hz is one octave (12 semitones) above 440 Hz
  st <- hz_to_semitone(880, ref_source = "A4", as_units = FALSE)
  expect_equal(st, 12, tolerance = 0.001)

  # Should also work without specifying ref_source (default)
  st_default <- hz_to_semitone(880, as_units = FALSE)
...
test_that("ref_freq parameter overrides ref_source", {
  ...
  st1 <- hz_to_semitone(880, ref_freq = 440, ref_source = "UEP83", as_units = FALSE)
  st2 <- hz_to_semitone(880, ref_freq = 440, as_units = FALSE)
...
test_that("semitone_to_hz works with different reference sources", {
  skip_if_not_installed("superassp")

  # UEP83: 12 ST above 110 Hz = 220 Hz
  hz_uep <- semitone_to_hz(12, ref_source = "UEP83", as_units = FALSE)
  expect_equal(hz_uep, 220, tolerance = 0.01)

  # Praat: 12 ST above 100 Hz = 200 Hz
  hz_praat <- semitone_to_hz(12, ref_source = "Praat", as_units = FALSE)
  expect_equal(hz_praat, 200, tolerance = 0.01)

  # A4: 12 ST above 440 Hz = 880 Hz
  hz_a4 <- semitone_to_hz(12, ref_source = "A4", as_units = FALSE)
...
test_that("round-trip conversion works with all reference sources", {
  ...
  for (ref_src in c("UEP83", "Praat", "A4")) {
    for (freq in test_freqs) {
      st <- hz_to_semitone(freq, ref_source = ref_src, as_units = FALSE)
      freq_recovered <- semitone_to_hz(st, ref_source = ref_src, as_units = FALSE)
...
test_that("UEP83 reference produces expected phonetogram values", {
  ...
  semitones <- hz_to_semitone(uep_freqs, ref_source = "UEP83", as_units = FALSE)
...
test_that("Praat reference useful for speech F0 analysis", {
  ...
  male_st_low <- hz_to_semitone(male_low, ref_source = "Praat", as_units = FALSE)
  male_st_high <- hz_to_semitone(male_high, ref_source = "Praat", as_units = FALSE)
  female_st_low <- hz_to_semitone(female_low, ref_source = "Praat", as_units = FALSE)
  female_st_high <- hz_to_semitone(female_high, ref_source = "Praat", as_units = FALSE)
...
test_that("ref_source parameter validation works", {
  ...
  expect_no_error(hz_to_semitone(220, ref_source = "UEP83", as_units = FALSE))
  expect_no_error(hz_to_semitone(200, ref_source = "Praat", as_units = FALSE))
  expect_no_error(hz_to_semitone(880, ref_source = "A4", as_units = FALSE))

  # Invalid ref_source should error
  expect_error(hz_to_semitone(220, ref_source = "invalid"))
})
