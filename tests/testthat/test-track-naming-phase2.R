...
test_that("Template expansion works for LP coefficients", {
  ...
  lpc <- trk_lpc(test_wav, analysisOrder = 12, toFile = FALSE, verbose = FALSE)
  ...
  # analysisOrder = 12 yields 13 predictor coefficients (a_0..a_12).
  # Column count: frame_time + RMS + gain + 13 LPC coeffs = 16
  expect_true("LPC13" %in% names(df))
...
test_that("clean_names parameter works", {
  ...
  fms <- trk_formant_forest(test_wav, numFormants = 2, toFile = FALSE, verbose = FALSE)
...
test_that("units are assigned correctly", {
  ...
  expect_true(is.numeric(df_no_units$F1_Hz))  # forest emits integer Hz
...
test_that("plotmath expressions are generated correctly", {
  ...
  expect_true(is.call(expr_f1))  # base call objects have no explicit S3 class
...
test_that("plotmath handles special cases correctly", {
  ...
  pitch <- trk_pitch_ksv(test_wav, toFile = FALSE, verbose = FALSE)
  ...
  expect_true(is.expression(expr_time) || is.call(expr_time))
})
