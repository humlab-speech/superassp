...
test_that("ucnv_phon_to_sone works at reference value (40 phon = 1 sone)", {
  skip_if_not_installed("superassp")

  # Reference value: 1 sone = 40 phons by definition
  expect_equal(ucnv_phon_to_sone(40), 1.0, tolerance = 0.01)
  expect_equal(ucnv_phon_to_sone(40, method = "moore-glasberg"), 1.0, tolerance = 0.01)
})

test_that("ucnv_sone_to_phon works at reference value (1 sone = 40 phon)", {
  skip_if_not_installed("superassp")

  # Reference value: 1 sone = 40 phons by definition
  expect_equal(ucnv_sone_to_phon(1), 40, tolerance = 0.01)
  expect_equal(ucnv_sone_to_phon(1, method = "moore-glasberg"), 40, tolerance = 0.01)
})
...
test_that("doubling property: +10 phon ≈ 2× sone", {
  ...
  sone_40 <- ucnv_phon_to_sone(40)
  sone_50 <- ucnv_phon_to_sone(50)
  sone_60 <- ucnv_phon_to_sone(60)
...
test_that("doubling property: 2× sone ≈ +10 phon", {
  ...
  phon_1 <- ucnv_sone_to_phon(1)
  phon_2 <- ucnv_sone_to_phon(2)
  phon_4 <- ucnv_sone_to_phon(4)
...
test_that("round-trip conversion preserves values (Zwicker)", {
  ...
  for (phon_orig in test_phons) {
    sone <- ucnv_phon_to_sone(phon_orig, method = "zwicker")
    phon_recovered <- ucnv_sone_to_phon(sone, method = "zwicker")
...
test_that("round-trip conversion preserves values (Moore-Glasberg)", {
  ...
  for (phon_orig in test_phons) {
    sone <- ucnv_phon_to_sone(phon_orig, method = "moore-glasberg")
    phon_recovered <- ucnv_sone_to_phon(sone, method = "moore-glasberg")
...
test_that("round-trip conversion from sone (Zwicker)", {
  ...
  for (sone_orig in test_sones) {
    phon <- ucnv_sone_to_phon(sone_orig, method = "zwicker")
    sone_recovered <- ucnv_phon_to_sone(phon, method = "zwicker")
    expect_equal(sone_recovered, sone_orig, tolerance = 0.01)
  }
})

test_that("ucnv_phon_to_sone is monotonically increasing", {
  skip_if_not_installed("superassp")

  phons <- seq(0, 100, by = 5)
  sones <- ucnv_phon_to_sone(phons)

  # Check that sone values increase monotonically
  expect_true(all(diff(sones) > 0))
})

test_that("ucnv_sone_to_phon is monotonically increasing", {
  skip_if_not_installed("superassp")

  sones <- 10^seq(-2, 2, by = 0.2)  # 0.01 to 100 sones
  phons <- ucnv_sone_to_phon(sones)

  # Check that phon values increase monotonically
  expect_true(all(diff(phons) > 0))
})

test_that("vectorization works for ucnv_phon_to_sone", {
  skip_if_not_installed("superassp")

  phons <- c(20, 40, 60, 80)
  result <- ucnv_phon_to_sone(phons)
...
test_that("vectorization works for ucnv_sone_to_phon", {
  skip_if_not_installed("superassp")

  sones <- c(0.5, 1.0, 2.0, 4.0)
  result <- ucnv_sone_to_phon(sones)
...
test_that("ucnv_phon_to_sone handles low values correctly", {
  skip_if_not_installed("superassp")

  # Below 40 phon uses different formula
  sone_0 <- ucnv_phon_to_sone(0)
  sone_10 <- ucnv_phon_to_sone(10)
  sone_20 <- ucnv_phon_to_sone(20)
  sone_30 <- ucnv_phon_to_sone(30)
...
test_that("ucnv_sone_to_phon handles low values correctly", {
  skip_if_not_installed("superassp")

  # Below 1 sone uses different formula
  phon_0.1 <- ucnv_sone_to_phon(0.1)
  phon_0.3 <- ucnv_sone_to_phon(0.3)
  phon_0.5 <- ucnv_sone_to_phon(0.5)
  phon_0.8 <- ucnv_sone_to_phon(0.8)
...
test_that("input validation for ucnv_phon_to_sone", {
  skip_if_not_installed("superassp")

  # Non-numeric input
  expect_error(ucnv_phon_to_sone("40"), "must be numeric")

  # NA values
  expect_error(ucnv_phon_to_sone(c(40, NA, 60)), "contains NA")

  # Negative values
  expect_error(ucnv_phon_to_sone(-10), "must be non-negative")
})

test_that("input validation for ucnv_sone_to_phon", {
  skip_if_not_installed("superassp")

  # Non-numeric input
  expect_error(ucnv_sone_to_phon("1"), "must be numeric")

  # NA values
  expect_error(ucnv_sone_to_phon(c(1, NA, 2)), "contains NA")

  # Zero or negative values
  expect_error(ucnv_sone_to_phon(0), "must be positive")
  expect_error(ucnv_sone_to_phon(-1), "must be positive")
})
...
test_that("warning for extreme phon values", {
  ...
  expect_warning(ucnv_phon_to_sone(130), "exceed 120")
})
...
test_that("warning for extreme sone values", {
  ...
  expect_warning(ucnv_sone_to_phon(500), "exceed 340")
})
...
test_that("Zwicker and Moore-Glasberg methods give similar results", {
  ...
  for (phon in test_phons) {
    sone_zwicker <- ucnv_phon_to_sone(phon, method = "zwicker")
    sone_mg <- ucnv_phon_to_sone(phon, method = "moore-glasberg")
...
test_that("Moore-Glasberg interpolation works", {
  ...
  sone_40 <- ucnv_phon_to_sone(40, method = "moore-glasberg")
  sone_42.5 <- ucnv_phon_to_sone(42.5, method = "moore-glasberg")
  sone_45 <- ucnv_phon_to_sone(45, method = "moore-glasberg")
...
test_that("ucnv_db_and_hz_to_sone combines conversions correctly", {
  skip_if_not_installed("superassp")

  # At 1000 Hz reference
  sone_40db <- ucnv_db_and_hz_to_sone(40, 1000)
  expect_equal(sone_40db, 1.0, tolerance = 0.01)

  # 50 dB at 1 kHz should be ~2 sones (+10 dB ≈ 2× loudness)
  sone_50db <- ucnv_db_and_hz_to_sone(50, 1000)
  expect_equal(sone_50db, 2.0, tolerance = 0.1)

  # Compare to step-by-step conversion
  phon <- ucnv_db_and_hz_to_phon(60, 2000)
  sone_manual <- ucnv_phon_to_sone(phon)
  sone_direct <- ucnv_db_and_hz_to_sone(60, 2000)
  expect_equal(sone_direct, sone_manual, tolerance = 0.01)
})

test_that("ucnv_sone_and_hz_to_db combines conversions correctly", {
  skip_if_not_installed("superassp")

  # At 1000 Hz reference
  db_1sone <- ucnv_sone_and_hz_to_db(1, 1000)
  expect_equal(db_1sone, 40, tolerance = 0.1)

  # 2 sones at 1 kHz should be ~50 dB (2× loudness ≈ +10 dB)
  db_2sone <- ucnv_sone_and_hz_to_db(2, 1000)
  expect_equal(db_2sone, 50, tolerance = 0.5)

  # Compare to step-by-step conversion
  phon <- ucnv_sone_to_phon(3)
  db_manual <- ucnv_phon_and_hz_to_db(phon, 500)
  db_direct <- ucnv_sone_and_hz_to_db(3, 500)
...
test_that("round-trip with frequency conversions", {
  ...
  for (i in seq_along(freqs)) {
    sone <- ucnv_db_and_hz_to_sone(spls[i], freqs[i])
    spl_recovered <- ucnv_sone_and_hz_to_db(sone, freqs[i])
...
test_that("frequency affects loudness perception", {
  ...
  sone_100hz <- ucnv_db_and_hz_to_sone(60, 100)
  sone_1000hz <- ucnv_db_and_hz_to_sone(60, 1000)
  sone_4000hz <- ucnv_db_and_hz_to_sone(60, 4000)
...
test_that("same loudness requires different SPL at different frequencies", {
  ...
  db_100hz <- ucnv_sone_and_hz_to_db(target_sone, 100)
  db_1000hz <- ucnv_sone_and_hz_to_db(target_sone, 1000)
  db_4000hz <- ucnv_sone_and_hz_to_db(target_sone, 4000)
...
test_that("piecewise formula transition is smooth at 40 phon", {
  ...
  sones <- ucnv_phon_to_sone(phons_near_40)
...
test_that("piecewise formula transition is smooth at 1 sone", {
  ...
  phons <- ucnv_sone_to_phon(sones_near_1)
...
test_that("conversion respects Stevens' power law", {
  ...
  sone_40 <- ucnv_phon_to_sone(phon_40)
  sone_50 <- ucnv_phon_to_sone(phon_50)
