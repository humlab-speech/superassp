test_that("S7 dispatch works for trk_rms with character input", {
  test_wav <- system.file("samples", "sustained", "a1.wav", package = "superassp")
  skip_if(test_wav == "", "Test file not found")

  # Should work with file path (character vector)
  result <- trk_rms(test_wav, toFile = FALSE, verbose = FALSE)
...
test_that("S7 dispatch maintains backward compatibility", {
  ...
  result1 <- trk_pitch_rapt(test_wav, toFile = FALSE, verbose = FALSE)
  expect_s3_class(result1, "AsspDataObj")

  # Pattern 2: Multiple files
  test_files <- rep(test_wav, 2)
  result2 <- trk_pitch_rapt(test_files, toFile = FALSE, verbose = FALSE)
  expect_type(result2, "list")
  expect_length(result2, 2)

  # Pattern 3: Custom parameters
  result3 <- trk_pitch_rapt(test_wav, minF = 75, maxF = 300,
...
test_that("S7 dispatch works with AVAudio list", {
  ...
  result1 <- trk_pitch_rapt(audio1, toFile = FALSE, verbose = FALSE)
  result2 <- trk_pitch_rapt(audio2, toFile = FALSE, verbose = FALSE)
