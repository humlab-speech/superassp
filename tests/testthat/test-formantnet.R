test_that("trk_formant_formantnet returns valid AsspDataObj for sustained /a/", {
  ...
  expect_true(all(c("fm", "bw") %in% names(result)))
  expect_equal(ncol(result$fm), 3L)
  expect_equal(ncol(result$bw), 3L)
  expect_true(nrow(result$fm) > 0L)

  # Sustained /a/ vowel: F1 ~ 700–900 Hz, F2 ~ 1000–1500 Hz
  f1_vals <- result$fm[result$fm[, 1] > 0, 1]
  f2_vals <- result$fm[result$fm[, 2] > 0, 2]
  ...
  # Bandwidths: positive, in reasonable range
  expect_true(all(result$bw > 0, na.rm = TRUE))
  expect_true(all(result$bw < 5001, na.rm = TRUE))

  # Frame rate 200 Hz
  expect_equal(attr(result, "sampleRate"), 200.0)
})

test_that("trk_formant_formantnet writes SSFF file with .fnf extension", {
  test_wav <- system.file("samples", "sustained", "a1.wav", package = "superassp")
  skip_if(test_wav == "", "Test file not found")
  skip_if(!superassp:::ort_available_cpp(), "ONNX Runtime not available")
  ...
  expect_equal(n, 1L)
  out_file <- file.path(out_dir, "a1.fnf")
...
test_that("trk_formant_formantnet function attributes are correct", {
  expect_equal(attr(trk_formant_formantnet, "ext"),        "fnf")
  expect_equal(attr(trk_formant_formantnet, "tracks"),     c("fm", "bw"))
  expect_equal(attr(trk_formant_formantnet, "outputType"), "SSFF")
})

test_that("trk_formant_formantnet respects numFormants", {
  test_wav <- system.file("samples", "sustained", "a1.wav", package = "superassp")
  skip_if(test_wav == "", "Test file not found")
  skip_if(!superassp:::ort_available_cpp(), "ONNX Runtime not available")
  ...
  r1 <- trk_formant_formantnet(test_wav, numFormants = 1L,
                                toFile = FALSE, verbose = FALSE)
  r5 <- trk_formant_formantnet(test_wav, numFormants = 5L,
                                toFile = FALSE, verbose = FALSE)
  expect_equal(ncol(r1$fm), 1L)
  expect_equal(ncol(r5$fm), 5L)
})

test_that("trk_formant_formantnet handles time windowing", {
  test_wav <- system.file("samples", "sustained", "a1.wav", package = "superassp")
  skip_if(test_wav == "", "Test file not found")
  skip_if(!superassp:::ort_available_cpp(), "ONNX Runtime not available")
  ...
  full  <- trk_formant_formantnet(test_wav, beginTime = 0, endTime = 0,
                                   toFile = FALSE, verbose = FALSE)
  win   <- trk_formant_formantnet(test_wav, beginTime = 0, endTime = 0.2,
                                   toFile = FALSE, verbose = FALSE)
