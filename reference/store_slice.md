...
This function allow you associate a feature vector of high
dimensionality to the part of the signal from which is computed. The
part can be a time slice (with defined start and end times), or the
entire signal. The collection of time windows from which data has been
obtained is viewed as sparse collection of slices. A slice defined by
its start and end times. The key features of the idea of sparse
collection of slices is that even though the set of features extracted
for the slice collection is consistent, a consistent spacing of slices
against the time line of the signal file is not required. This design
choice helps with the storage of output of computationally intensive
procedures that summarize a whole recording or just the part of it where
...
- values:
  ...
  A list of values to be stored. The identity of each measurement value
  should be indicates as informative list names, as it will help
...
- start_sample:
  ...
  The first sample of the signal file that was submitted for analysis,
  and for which the results should now be stored. If `NULL`, all samples
  from the first sample to the `end_sample` will be included.

- end_sample:

  The last sample of the signal file that was submitted for analysis,
  and for which the results should now be stored. If `NULL`, all samples
  until the end of the media file will be used.

- fileExtention:

  This file extension will be used when making the sparse slice name
  from the `mediaFileName`. Defaults to ".sli".
...
slices is a large collection of acoustic measurements of voice obtained
from just the portion of the recording where the participant produces a
...
well suited to be aligned to a particularportion of the speech signal,
and in this case `start_time` and `start_time` is not set (`NULL`). Even
though the the feature set is alined to a maximally large portion of the
speech signal, it is still considered a slice in the sparse collection
of slices.
...
1.  The results stored in a sparse collection of slices must be
    identially shaped. That is, they may only hold the features defined
    when you first insert data into them. It is of course possible to
    have multiple sparse collections (with unique file extensions) where
    the user may store outputs of different analyses associated with a
    single speech signal.

2.  Two slices may not be associated with exacly the same portion of the
    speech signal. If the user supplies a new result to the same time
    ...
    use multiple sparse slice collection files.
