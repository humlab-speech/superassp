...
- `src/assp/`, `src/SPTK/`, `src/ESTK/`, `src/tcl-snack/` (vendored upstream — see `inst/VENDORED_SOURCES.md`)
...
- SPTK (`src/SPTK/`, vendored)
- ESTk (`src/ESTK/`, vendored)
...
Vendored upstream code (patch the fork, then re-vendor — see `inst/VENDORED_SOURCES.md`):
- `src/SPTK/`, `src/ESTK/`, `src/tcl-snack/`, `src/Yin-Pitch-Tracking/`, `src/pyin/`, `src/tandem/`

Configuration (edit carefully):
- `src/Makevars` — Compilation config (critical)
- `inst/VENDORED_SOURCES.md` — vendored upstream revisions
