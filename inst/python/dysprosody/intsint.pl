...
print STDOUT "; INTSTINT labels created on $now by $script_name $version\n";
print STDOUT ";    $nval values  mean = $linear_mean_f0\n";
...
print STDOUT "optimising range between $MIN_RANGE and $MAX_RANGE (in octaves) by step $STEP_RANGE\n";
print STDOUT "optimising mid between $min_mean and $max_mean (in Hz) by step $STEP_SHIFT\n";
...
print STDOUT "<parameter range=$best_range>\n";
print STDOUT "<parameter key=$key>\n";

for $i (0..$nval-1) {
			$f0 = int(linear($f0[$i])+0.5);
			$estimate = int(linear($best_estimate[$i])+0.5);
			print STDOUT "$t[$i] $best_intsint[$i] $f0 $estimate\n";
} #for $i
...
#version history
#2.12	28april2024 Altered the script to read from STDIN and write to STDOUT
