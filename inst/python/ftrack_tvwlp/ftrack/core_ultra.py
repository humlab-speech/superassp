"""
Ultra-optimized formant tracking: Vectorization + Numba + Cython.
...
- Phase 2B: Cython for pitch tracking (10-50x on loops)

Expected performance: ~1.5-2.0s total (2-2.4x real-time)
"""
...
# Try to use Cython-accelerated pitch tracking
try:
    from .gloat.pitch_fast import srh_pitch_tracking_fast as srh_pitch_tracking
    USE_CYTHON_PITCH = True
except ImportError:
    from .gloat.pitch import srh_pitch_tracking
    USE_CYTHON_PITCH = False

# Use Numba-accelerated GCI detection
from .gloat.gci_fast import sedreams_gci_detection_fast


def ftrack_tvwlp_ultra(s, fs, lptype='tvwlp_l2', nwin=None, nshift=None, p=8, q=3,
    ...
    - Numba JIT compilation for GCI detection (Phase 2A)
    - Cython compilation for pitch tracking (Phase 2B)

    Expected speedup: 2.5-3.0x vs original
    Expected runtime: ~1.5s for 3.6s audio (2.4x real-time)
    ...
    use_numba : bool
        Use Numba JIT for GCI if available (default: True)
    use_cython : bool
        Use Cython for pitch if available (default: True)
    ...
    if lptype_lower in ['tvwlp_l2', 'tvwlp_l1']:
        ...
        # Pitch tracking (Cython-accelerated if available)
        if USE_CYTHON_PITCH and use_cython:
            f0, vuv, srh_val = srh_pitch_tracking(s, fs, f0_min, f0_max, use_cython=True)
        else:
        ...
        # GCI detection (Numba-accelerated)
    ...
    if plot_flag:
        ...
        plt.title(f'Formant Tracks ({lptype}) - ULTRA (Vectorized + Numba + Cython)')
        plt.show()
...
# Print optimization status on import
print(f"Ultra-optimized formant tracking:")
print(f"  ✓ Vectorized TVLP")
print(f"  ✓ Numba GCI detection")
if USE_CYTHON_PITCH:
    print(f"  ✓ Cython pitch tracking")
else:
    print(f"  ⚠ Cython pitch tracking not available")
