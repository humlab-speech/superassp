...
#endif
{
  ...
  if(n > n0) {
    ...
    if(!(din = (float*)ckalloc(sizeof(float)*n))) {
      REprintf("Allocation problems in xget_window()\n");
      return(FALSE);
    }
...
#endif
{
  ...
  case 3:			/* Hanning */
    ...
    break;
  default:
    REprintf("Unknown window type (%d) requested in window()\n",type);
    return(FALSE);
  }
  return(TRUE);
}
...
#endif
{
  ...
  if(nwind < size) {
    ...
    if(!dwind) {
      REprintf("Can't allocate scratch memory in wind_energy()\n");
      return(0.0);
    }
  }
  if(nwind != size) {
...
#endif
{
  ...
  if(nwind != wsize) {
    ...
    if(!dwind) {
      REprintf("Can't allocate scratch memory in lpc()\n");
      return(FALSE);
    }
    nwind = wsize;
  }
...
#endif
{
  ...
  if((total = size+start+nlags) > dbsize) {
    ...
    if(!(dbdata = (float*)ckalloc(sizeof(float)*total))) {
      REprintf("Allocation failure in crossf()\n");
      return;/*exit(-1);*/
    }
    dbsize = total;
  }
...
#endif
{
  ...
  if((total = size+start0+nlags0) > dbsize) {
    ...
    dbsize = 0;
    if(!(dbdata = (float*)ckalloc(sizeof(float)*total))) {
      REprintf("Allocation failure in crossfi()\n");
      return;/*exit(-1);*/
    }
    dbsize = total;
  }
