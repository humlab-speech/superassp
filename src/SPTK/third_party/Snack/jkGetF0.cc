...
#include <stdio.h>
#include <limits.h>
#include <R_ext/Print.h>   /* Rprintf/REprintf: CRAN forbids stdout/stderr writes */
#if 1
...
Get_f0(Sound *sound, Tcl_Interp *interp, int objc, Tcl_Obj *CONST objv[])
{
  ...
  if (debug_level)
    REprintf("init_dp_f0 returned buff_size %ld, sdstep %ld.\n",buff_size, sdstep);
...
/* Reset flags for function-local statics in do_ffir() and downsample().
   free_dp_f0() sets these so the next call re-allocates from scratch. */
static int do_ffir_reset = 0;
static int downsample_reset = 0;
#endif
...
#endif
{
  ...
  if(input && (samsin > 0) && (decimate > 0) && *samsout) {
    ...
    if(downsamp(input,foutput,samsin,samsout,state_idx,decimate,ncoefft,b,init)) {
      ...
      REprintf("Problems in downsamp() in downsample()\n");
#if 0
  } else
    REprintf("Bad parameters passed to downsample()\n");
#else
  }
#endif
...
#endif
{
  if(in && out) {
    ...
    Rprintf("Bad signal(s) passed to downsamp()\n");
  return(FALSE);
}
...
float	*buf, *bufo;
float *fc;
int in_samps, ncoef, invert, skip, init, *out_samps;
int idx;
#else
static void do_ffir(float *buf, int in_samps, float *bufo, int *out_samps,
                    int idx, int ncoef, float *fc, int invert, int skip,
...
#endif
{
  ...
  /* free_dp_f0() freed co and mem (via mem2 alias) but could not reset
     these function-local statics.  Force re-allocation on next use. */
  if (do_ffir_reset) {
    mem = NULL;
    fsize = 0;
    resid = 0;
  ...
  if(ncoef > fsize) {/*allocate memory for full coeff. array and filter memory */    fsize = 0;
    ...
    if(!((co = (float *)ckrealloc((void *)co, sizeof(float)*i)) &&
	 (mem = (float *)ckrealloc((void *)mem, sizeof(float)*i)))) {
      REprintf("allocation problems in do_fir()\n");
      return;
    }
    fsize = ncoef;
#if 1
 ...
 */
  ...
  *sdstep = nframes * step;
  ...
  if(debug_level){
    REprintf("done with initialization:\n");
    REprintf(
...
#endif
{
  ...
  if (decimate <= 1)
    ...
    if (!dsdata) {
      REprintf("can't get downsampled data.\n");
      return 1;
    }
  }
  ...
  if (!stat) { 
    REprintf("can't get stationarity\n");
    return(1);
  }
  sta = stat->stat;
  ...
  for(i = 0; i < nframes; i++) {
	...
	return(1);
      ...
      if(debug_level) 
	REprintf("allocating %d more frames for DP circ. buffer.\n", size_cir_buffer);
	...
	*sp1++ = *sp2++;
    ...
    if (debug_level >= 2) {
      REprintf("%d engref:%10.0f max:%7.5f loc:%4d\n",
  ...
  *vecsize = 0;			/* # of output frames returned */
  ...
  if( num_active_frames >= size_frame_hist  || last_time ){
    ...
    if(debug_level)
      REprintf("available frames for backtracking: %d\n",
num_active_frames);
...
void free_dp_f0()
{
  ...
  /* Signal function-local statics to re-initialize on next call.
     do_ffir::mem was freed above (via mem2 alias) but its local fsize
     guard was not reset — without this, the next call skips realloc
