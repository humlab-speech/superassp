...
bool CodecApi<CodecImplementation>::ReadAudioData(
  ...
  if (samples == NULL) {
    REprintf("CodecApi::ReadAudioData: empty pointer was given");
    return true;
  }
  ...
  if (fseek(fr->fp(), offset_audio_container, SEEK_SET) != 0) {
    REprintf("CodecApi::ReadAudioData: error seeking the beginning of the "
            "audio container");
    return false;
  }
  return status;
...
bool CodecApi<CodecImplementation>::Load(
  ...
  if (fr != NULL) {
    ...
    REprintf("CodecApi::Load: Failed to open \"%s\"", filename.c_str());
    return false;
  }
}
