...
bool WavRiffCodec::ReadHeader(FileResource *fr,
  ...
  if (riff_type == "bext") {
    ...
    if (!CheckChunk(fr, "fmt ")) {
      REprintf("Invalid file: fmt subtype not found in bext header");
      return false;
    }
  }
  ...
  if (fread(static_cast<void *>(&len), sizeof(len), 1, fr->fp()) != 1) {
    REprintf("failed to skip file length");
    return false;
  }
  if (len < 16) {  // bad format chunk length
    REprintf("Invalid of the wave format buffer");
    return false;
  }
  ...
  if (fread(static_cast<void *>(&n_channels), sizeof(n_channels), 1, fr->fp()) != 1) {
    REprintf("Failed to read number of channels");
    return false;
  }
  if (n_channels != 1) {
    REprintf("Attempt to load multi channel audio");
    return false;
  }
  int32_t sample_rate;
  if (fread(&sample_rate, sizeof(sample_rate), 1, fr->fp()) != 1) {
    REprintf("Failed to read sample rate");
    return false;
  }
  ...
  if (fr->eof()) {
    REprintf("Unexpected end of file: no data");
    return false;
  }

  int32_t num_bytes = 0;
  if (fread(&num_bytes, sizeof(num_bytes), 1, fr->fp()) != 1) {
    REprintf("Failed to read number of bytes");
    return false;
  }
...
bool WavRiffCodec::ReadAudioData(int32_t wave_start,
...
#endif
      REprintf("WaveIO::ReadSamples: only %d out of %d values read",
          read, num_samples);
      status = false;
    }
  }
  ...
  break;
  default:
    REprintf("WaveIO::ReadSamples: Unsupported coding type (%d)",
            coding_type_);
    status = false;
  }
  return status;
