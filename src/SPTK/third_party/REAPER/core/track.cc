...
bool Track::Save(FileResource *fr) const {
  ...
  for (int i = 0; i < num_frames(); ++i) {
    ...
    if (fwrite(&f, sizeof(f), 1, fr->fp()) != 1) {
      REprintf("Failed to write to track file");
      return false;
    }
  }
...
void Track::SetTimes(float *times, int length) {
  if (length != num_frames_) {
    REprintf("Track::SetTimes: input `times` has different number "
...
#endif
    REprintf("Track::SetVoicing: input has different number "
...
Track *Track::GetSubTrack(float start,
  ...
  if (ch_size == -1 && ch_offset == -1) {
    ...
    if (ch_size > this->num_channels()) {
      REprintf("Incorrect number of channels for sub track");
      return NULL;
    }
  }
...
Track *Track::GetSubTrack(int start_frame_index,
  ...
  if (start_frame_index > end_frame_index ||
    ...
    REprintf("Incorrect frame indices for sub-track 0<%d<=%d<%d\n",
                start_frame_index,
                end_frame_index,
                this->num_frames());
    return NULL;
  ...
  if (start_channel_index > end_channel_index ||
    ...
    REprintf("Incorrect channel indices for sub-track 0<%d<=%d<%d\n",
                start_channel_index,
                end_channel_index,
                this->num_channels());
    return NULL;
...
bool Track::SetCombinedTrack(const Track &track_a, const Track &track_b) {
  if (track_a.num_frames() != track_b.num_frames()) {
    REprintf("Mismatching number of frames: %d vs. %d",
                track_a.num_frames(),
                track_b.num_frames());
    return false;
  }
