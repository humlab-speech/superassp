##' Probabilistic YIN Pitch Tracking (Simplified C++ implementation)
##'
##' @description Extract F0 (fundamental frequency) using a simplified probabilistic YIN.
##'   This is a C++ implementation that is significantly faster than the Python/librosa
##'   version and requires no Python dependencies.
##'
##'   The probabilistic YIN algorithm \insertCite{Mauch.2014.10.1109/icassp.2014.6853678}{superassp}
##'   extends YIN \insertCite{Cheveigné.2002.10.1121/1.1458024}{superassp} by considering
##'   multiple pitch candidates. This simplified version provides the core YIN functionality
##'   without the full HMM inference.
##'
...
##' @param explicitExt Output file extension (default: "pyp")
...
trk_pyin <- function(listOfFiles,
  ...
  for (i in seq_len(n_files)) {
    ...
    tryCatch({
      ...
      out_obj <- create_pyin_asspobj(pyin_result, windowShift)
...
##' Helper function to create AsspDataObj from pYIN results
##' @keywords internal
##' @noRd
create_pyin_asspobj <- function(pyin_result, windowShift) {
  n_frames <- pyin_result$n_frames
  sample_rate <- pyin_result$sample_rate
  frame_rate <- 1000.0 / windowShift  # windowShift is in ms

  # Create AsspDataObj structure with two tracks
  obj <- list()
  obj$F0 <- pyin_result$f0
  obj$prob <- pyin_result$probability
