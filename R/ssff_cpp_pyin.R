...
##' implementation of the probabilistic YIN algorithm
##' \insertCite{Mauch.2014.10.1109/icassp.2014.6853678}{superassp}, which extends
##' YIN \insertCite{Cheveigné.2002.10.1121/1.1458024}{superassp} by evaluating
##' multiple pitch candidates. More robust than plain YIN in noise and lower-voiced
##' speech; the \code{prob} track can serve as a soft voicing mask.
##'
...
##' @param threshold Numeric. YIN dip threshold (0–1); lower values accept weaker
...
##'   Default \code{FALSE}.
##' @param explicitExt Character. Output file extension. Default \code{"pyp"}.
...
create_pyin_asspobj <- function(pyin_result, windowShift) {
  ...
  new_asspdataobj(
    tracks = list(F0 = pyin_result$f0, prob = pyin_result$probability),
