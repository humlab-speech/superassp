...
#'
#' Reads audio into an AsspDataObj for in-memory DSP processing.
#' Native formats (wav, au, kay, nist, nsp) are read via the C-level
#' [read_ssff()] for maximum speed. All other formats (mp3, mp4, aac,
#' flac, ogg, …) use [av::read_audio_bin()] as the fallback.
#' Resampling (via `target_sample_rate`) always uses the av path.
#'
...
av_to_asspDataObj <- function(file_path, start_time = 0, end_time = NULL,
  ...
  # C-level primary path: fast, no format conversion for native formats.
  # Skip if resampling is requested (read_ssff does not resample).
  if (file_ext %in% native_exts && is.null(target_sample_rate)) {
    begin <- start_time
    end   <- if (is.null(end_time)) 0 else end_time
    result <- tryCatch(
...
#' @param analysis_function Name of analysis function (e.g., "trk_rms", "trk_formant_forest")
#' @param start_time Start time in seconds (default 0)
...
#' }
#' @keywords internal
process_media_file <- function(file_path, analysis_function = "trk_rms",
