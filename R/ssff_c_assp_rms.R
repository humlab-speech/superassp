...
##'
##' Computes the short-term root-mean-square (RMS) amplitude of audio signals
##' using the *libassp* C library \insertCite{s5h}{superassp}. By default,
...
##' @param linear Logical. If \code{TRUE}, return linear RMS amplitude instead of
##'   dB. Default \code{FALSE} (dB scale).
##'
##' @return If \code{toFile = FALSE}: an \code{AsspDataObj} with track:
##'   \describe{
##'     \item{\code{RMS[dB]}}{REAL32, dB (or linear amplitude if \code{linear = TRUE}),
##'       n_frames x 1 column. Short-term RMS amplitude per frame.}
##'   }
##'   Frame rate: \code{1000 / windowShift} Hz (default 200 Hz).
##'   If \code{toFile = TRUE}: integer count of files written, returned invisibly.
##'
##' @details
##' Window function defaults to HAMMING (not BLACKMAN as in other libassp functions).
##' Set \code{linear = TRUE} for raw amplitude values without log conversion.
##'
##' @seealso [wrassp::rmsana]
##'
##' @useDynLib superassp, .registration = TRUE
##' @importFrom Rcpp sourceCpp
##' @usage trk_rms(listOfFiles, beginTime = 0, centerTime = FALSE, endTime = 0, windowShift = 5, windowSize = 20, effectiveLength = TRUE, linear = FALSE, window = "HAMMING", toFile = TRUE, explicitExt = "rms", outputDirectory = NULL, assertLossless = NULL, logToFile = FALSE, convertOverwrites = FALSE, keepConverted = FALSE, verbose = TRUE)
...
# Function attributes
attr(trk_rms, "ext") <- "rms" 
attr(trk_rms, "tracks") <- "RMS[dB]"
attr(trk_rms, "outputType") <- "SSFF"
attr(trk_rms, "nativeFiletypes") <- c("wav", "au", "kay", "nist", "nsp")
attr(trk_rms, "suggestCaching") <- FALSE
