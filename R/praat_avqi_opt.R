...
#'
#' # Compute AVQI
#' result <- praat_avqi_opt(sv, cs,
#'                          speaker.name = "John Doe",
#'                          speaker.ID = "001")
#' }
praat_avqi_opt <- function(svDF,
                           csDF,
                           min.sv = 1000,
                           speaker.name = NULL,
...
attr(praat_avqi_opt, "outputType") <- c("list")
attr(praat_avqi_opt, "tracks") <- c("AVQI_VERSION", "Speaker", "ID", "CPPS", "HNR",
                                     "Shim_local", "Shim_local_DB", "LTAS_Slope",
                                     "LTAS_Tilt", "AVQI")
attr(praat_avqi_opt, "ext") <- c("avqi")
