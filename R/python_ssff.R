...
pm_times, pm, f0_times, f0, corr = pyreaper.reaper(x=int_x, fs=fs, minf0 = fMin, maxf0 = fMax, do_high_pass=hp, do_hilbert_transform= ht,  frame_period=ws, inter_pulse=ws, unvoiced_cost =uc)" -> script
    Residual_symetry_string <- utils::capture.output( reticulate::py_run_string(script),type = "output")
...
#rapt(f,beginTime=0,endTime=1,toFile=FALSE) -> outportion
#rapt(f,beginTime=0,endTime=0,toFile=FALSE) -> out
#reaper(f,beginTime=0,endTime=1,toFile=FALSE) -> outportion
#reaper(f,beginTime=0,endTime=0,toFile=FALSE) -> out
