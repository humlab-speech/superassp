...
- Step b and c were repeated several times. Each time it generated a set of 64 MLPs. The set that gave the best performance on testing data (Sect. IV-A) was selected

- Training of the multiple-unit based MLP (Sect. III-C)
- Training data: The same data as above
- Training steps
- MATLAB neural-network toolbox was used for training
- For each mixture, a T-F soft mask is first generated using the single-unit based MLP
- Given a soft mask, for each T-F unit uc,m, the probabilities of the surrounding T-F units are used as inputs to train an MLP to label uc,m. Ideal binary masks provide the desired outputs. We note that in the 64-channel version the frequency neighborhood size is reduced to 4 since only 64 gammatone filters are used. The time neighborhood size remains to be 2